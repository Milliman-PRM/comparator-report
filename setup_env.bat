rem ### CODE OWNERS: Ben Copeland, Umang Gupta
rem
rem ### OBJECTIVE:
...
echo %~nx0 %DATE:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: Setting up comparator-report env
...
echo.
echo %~nx0 %COMPARATOR_REPORT_HOME:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: Redirecting COMPARATOR_REPORT_HOME to local copy
SET COMPARATOR_REPORT_HOME=%~dp0%
echo %~nx0 %DATE:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: COMPARATOR_REPORT_HOME is now %COMPARATOR_REPORT_HOME%

echo.
echo %~nx0 %DATE:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: Prepending local copy of python library to PYTHONPATH
set PYTHONPATH=%COMPARATOR_REPORT_HOME%python;%PYTHONPATH%
echo %~nx0 %DATE:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: PYTHONPATH is now %PYTHONPATH%

echo %~nx0 %DATE:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: Finished setting up comparator-report env